...
- **64bit 専用**。32bit(i686)ビルドでは 3 モードの結果集合が一致しない(mode 1 trie が余分な id を返す。`size_t`/ポインタ幅依存の潜在バグ、CI の cibuildwheel i686 テストで顕在化)。index フォーマットも 64bit 前提なので、cibuildwheel は `*_i686 *-win32` を skip。64bit(macOS x86_64/arm64・Linux x86_64/aarch64・**Windows x86_64**)では 3 モード一致・golden 一致を確認済み。Windows(MSVC)は 0.2.0 でコード無改変のまま対応(LLP64 で `size_t` 8 バイトのため 32bit のような不一致は出ない)。
