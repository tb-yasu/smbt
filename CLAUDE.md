...
- **64bit 専用**。32bit(i686)ビルドでは 3 モードの結果集合が一致しない(mode 1 trie が余分な id を返す。`size_t`/ポインタ幅依存の潜在バグ、CI の cibuildwheel i686 テストで顕在化)。index フォーマットも 64bit 前提なので、cibuildwheel は `*_i686 *-win32` を skip。64bit(x86_64/arm64)では 3 モード一致・golden 一致を確認済み。
